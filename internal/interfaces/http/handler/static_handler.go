...
<div class="wrap"><div class="card"><div class="toolbar"><div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center"><input id="search" type="text" placeholder="名前/Channel IDで検索" oninput="renderUsers()" /><button class="md-btn" onclick="manualRefresh()"><span class="material-symbols-outlined" style="font-size:18px">sync</span> 即時更新</button><button class="md-btn" style="background:#b91c1c" onclick="stopMonitoring()"><span class="material-symbols-outlined" style="font-size:18px">stop_circle</span> 監視停止</button></div><div class="right"><div id="status" class="status">初回取得中...</div></div></div><div class="content" style="padding:12px 16px"><div class="meta" style="margin-bottom:8px"><span id="count">0</span> 名 <span id="updated"></span> <span style="margin-left:12px;font-size:11px;opacity:.75">SSE更新</span></div><div id="userList"><div class="empty">データを読み込んでいます…</div></div></div></div></div>
<script>
let cachedUsers=[];let currentVideoId=localStorage.getItem('currentVideoId')||'';let es=null;let sseConnected=false;let retries=0;const maxBackoff=30000;
...
function connectSSE(){const st=document.getElementById('status');const upd=document.getElementById('updated');if(es) es.close();st.textContent='SSE接続中...';try{es=new EventSource('/api/sse/'+encodeURIComponent(currentVideoId)+'/users');es.addEventListener('connected',()=>{sseConnected=true;retries=0;st.className='status online';st.textContent='オンライン - 接続済み';});es.addEventListener('user_list',ev=>{const data=JSON.parse(ev.data);cachedUsers=Array.isArray(data.users)?data.users:[];st.className='status online';st.textContent='オンライン - コメントユーザー数: '+(data.count||cachedUsers.length);upd.textContent='（更新: '+new Date().toLocaleTimeString()+'）';renderUsers();});es.addEventListener('timeout',()=>{st.className='status offline';st.textContent='タイムアウト';scheduleReconnect();});es.addEventListener('monitoring_stopped',()=>{st.className='status offline';st.textContent='停止済み';});es.addEventListener('error',()=>{if(!sseConnected){st.className='status offline';st.textContent='接続失敗';}scheduleReconnect();});}catch(e){st.className='status offline';st.textContent='SSEエラー: '+e.message;scheduleReconnect();}}
...
function manualRefresh(){connectSSE();}
function renderUsers(){const list=document.getElementById('userList');const q=(document.getElementById('search').value||'').toLowerCase();let users=cachedUsers.slice();if(q){users=users.filter(u=>{const n=(u.display_name||u.displayName||'').toLowerCase();const c=(u.channel_id||u.channelID||'').toLowerCase();return n.includes(q)||c.includes(q);});}document.getElementById('count').textContent=users.length;if(users.length===0){list.innerHTML='<div class="empty">該当するユーザーがいません</div>';return;}list.innerHTML='<ol>'+users.map(u=>'<li>'+escapeHtml(u.display_name||u.displayName||'')+'</li>').join('')+'</ol>';} 
